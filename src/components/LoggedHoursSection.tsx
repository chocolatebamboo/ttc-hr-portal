 ...
 * (src/app/(portal)/time/TimesheetView.tsx), extracted into its own component so Availability
 * can render it as one of its own accordion sections (brief, Sept 2026: "Move the useful My
 * Time functionality into Availability... Choose Your Availability / Your Submissions / Logged
 * Hours / Time Off"). /time itself is now just a redirect to /availability — see that route's
 * own comment.
 *
 * No `employeeId` prop: the underlying fetch (`GET /api/time/timesheet`) is already
 * session-scoped server-side, same as it was on My Time.
 *
...
export default function LoggedHoursSection({ showHeading = true }: { showHeading?: boolean }) {
  ...
  // loading and the tap landing) — kept per-entry so the message shows next to the row it's
  // actually about, same idea as correctionError but scoped tighter since more than one row can
  // now be deletable at once.
...
/** One logged day, styled to match the widget/card list look used elsewhere on this page rather
 *  than a bordered table row. Only rendered for a day that actually has an entry — see the "no
 *  placeholder rows" comment above this component's call site. Only a Returned day gets an
 *  "Edit & resubmit" control — same rule TimesheetTable enforces.
 *
