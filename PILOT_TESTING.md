...
Five real accounts — one HR/Super Admin, one Supervisor, two or three Team Members who report to
...
mobile devices and browsers, and a supervisor and team members who don't already know where every
button is.
...
3. Run `npm run pilot:create-accounts`. Each person gets a Supabase invite email with a link to
   ...
   the script later (to add a third team member, fix a typo, etc.) is safe: it updates existing
...
- A team member's login email can't be changed from the Team Members page — that needs a matching
  change on their Supabase Auth account, which isn't wired up yet.
- Tables that scroll sideways on a narrow phone screen (the supervisor's per-team-member timesheet
...
it in — each step depends on data the previous one created (a team member submits a timesheet
before their supervisor can approve it, for instance), so the Team Member and Supervisor testers
...
- [ ] If Google sign-in is turned on (README "Getting set up" §9), sign out and try "Continue
      ...
      email than any invited team member, confirm trying that one bounces back to `/login` with a
...
- [ ] Open My Profile — upload a photo, edit your personal phone/email and emergency contact,
      ...
      these from the Team Members admin page instead, then confirm it shows up here too).
...
- [ ] Do everything in the Team Member checklist above first — a supervisor is also a team member
...
- [ ] Confirm you can NOT see or act on a team member who is not your report (ask HR to point you
...
- [ ] Before you approve anything above, confirm your own Onboarding nav dot and Dashboard
      ...
      approval — this is the same in-app notification a team member sees, just for reviewers.

### HR / Super Admin

- [ ] Post a company-wide Announcement, a department-only one, and one to a single individual;
      confirm each shows up only where it should (ask the Team Member/Supervisor testers to check).
- [ ] Upload a Document, assign it (try all three: global, one department, one specific person),
      ...
      wrong people can't.
- [ ] Start an Onboarding checklist for one of the pilot Team Members, then add at least one custom
      step of each type beyond the defaults — a plain Task, a Document (pick one that requires
      acknowledgment), and a Training or Meeting step — and confirm the team member only ever sees
...
- [ ] Under Onboarding's Manage tab, open **Manage Templates** and build a named template (e.g.
      ...
      Team Member's checklist by picking that template from the dropdown instead of the standard
      starter, and confirm their checklist comes in with exactly those steps, in order, with any
      due-date offsets applied correctly. Then delete the template and confirm that team member's
...
- [ ] After starting a checklist above, confirm the Manage tab's roster shows a status pill for
      that team member (Action Needed / Upcoming / Waiting on Team Member / Not Started / Completed),
      ...
      **30/60/90-Day Checkpoints** panel (3 fixed milestones) — check one readiness task and one
      checkpoint off, add a note to a checkpoint, and confirm both save. Ask that team member to
      confirm neither panel appears anywhere on their own Onboarding page.
- [ ] Add a Certification step to one pilot Team Member's checklist (or a template) and have them
      ...
      instead, then confirm the team member can retake it and the earlier attempt stays visible
...
- [ ] Open the Reports page and click "This week" or "This month," or pick a date range that
      covers what the Team Member testers submitted, then download the Payroll Hours CSV. Open it
      ...
      approved yet.
- [ ] On the same Reports page, use the Team Member picker to narrow the report to one pilot
      tester — confirm the preview table and the CSV both show only that person's row (and the
      CSV filename reflects it), then switch back to "All team members" and confirm the full table
      returns.
- [ ] Open Team Members — confirm every pilot tester shows up with the right role, department, and
      supervisor. Edit one person's job title or department and confirm it saves.
- [ ] From Team Members, add one more person (a throwaway test account you control) with **Add
      Team Member** and confirm the invite email arrives — this is now the in-app alternative to
      ...
      load Team Members — replaced by a green "Accepted [date/time]" badge, which should itself be
      gone if you check back in a couple weeks.
- [ ] Deactivate that throwaway account from Team Members and confirm they're immediately signed
...
- [ ] As a Super Admin, click **View as** on the Team Member or Supervisor pilot tester and confirm
...
- [ ] Open Attendance — confirm it shows every pilot team member for the current week (not just
      one supervisor's team), and that the awaiting-approval / missing-clock-out counts match
      what you'd expect from what the Team Member testers actually did.
- [ ] Open PTO Management — confirm the Pending queue shows requests from every pilot team member,
...
- [ ] Open Administration — add a throwaway department, rename it, confirm the new name shows
      up immediately on that department's team members (check Team Members or Directory). Confirm
      Delete is disabled (with an explanation) while a team member is still assigned to it, then
      reassign that team member elsewhere and confirm Delete now works.
- [ ] Everything in the Team Member and Supervisor checklists too, since HR can act as either.
