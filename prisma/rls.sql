...
-- Same three-way row shape as AvailabilitySubmission/DateTask (self, their supervisor, or an
-- admin may READ) — but write is deliberately narrower than DateTask's: a confirmed Shift is
-- exactly the record the client spec says a team member "must not be able to edit or delete
...
create policy shift_select on "Shift" for select using (
  is_admin()
  or "employeeId" = current_employee_id()
  or "employeeId" in (select id from "Employee" where "supervisorId" = current_employee_id())
);

drop policy if exists shift_write on "Shift";
create policy shift_write on "Shift" for all using (
  is_admin()
  or "employeeId" in (select id from "Employee" where "supervisorId" = current_employee_id())
) with check (
  is_admin()
